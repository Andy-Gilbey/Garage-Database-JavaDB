...
import ManageStock.DeleteStock;
...
public class ManageCarGui extends JFrame {
   ...
   /**
    * Item listener for the check-box in the gui. Checks if the state has been changed in order to enable or disable the remove button
    * @param ItemStateChanged e
    */
   protected void enableDeleteBoxItemStateChanged(ItemEvent e) {
      boolean flag = carListTable.getSelectionModel().isSelectionEmpty();
      if (flag) {
         JOptionPane.showMessageDialog(rootPane, "Nothing Selected", "Error!", JOptionPane.ERROR_MESSAGE);
      } else {
   ...
   /**
    * ActionListener for the Edit button. Utilises the buildCarEdit method to extract data from the gui and pass it onto the setEditDetails method
    * in order to update an entry in the db
    * @param ActionEvent e
    * @throws SQLException
    */
   ...
   /**
    * ActionListener for the close button. Calls the dispose method from the JFrame class to dispose the window.
    * @param ActionEvent e
    */
   ...
   protected void deleteBtnActionPerformed(ActionEvent e) throws SQLException, IntegrityConstraintValidation {
      Car car = new Car();
      car = buildCarSave();
      car = DeleteCar.integrityValidation(car);
      if (car != null) {
         ...
         if (selectedOption == JOptionPane.YES_OPTION) {
            DeleteCar.carDelete(car);
            dispose();
            updateTable(carListTable);
            JOptionPane.showMessageDialog(null, "Success " + car.getCarId() + " has been deleted", "Success", JOptionPane.INFORMATION_MESSAGE);
         ...
         JOptionPane.showMessageDialog(null, "Customer found in stock table. Cannot delete while customer is involed in a sale.", " Error", JOptionPane.ERROR_MESSAGE);
      }
